...
namespace streami {
  ...
  struct Plane {
    inline __both__ float eval(const vec3f v) const
    { return dot(v,N)-d; }
    inline __both__ float eval(const vec4f v) const
    { return dot((const vec3f&)v,N)-d; }
    
    vec3f N;
    float d;
  };
  inline __both__ Plane makePlane(const vec3f a,
                                     const vec3f b,
                                     const vec3f c)
  {
  ...
  inline __both__ Plane makePlane(const vec4f a,
                                     const vec4f b,
                                     const vec4f c)
  {
  ...
  inline __both__
  bool intersectPair(vec3f &value,
                     const vec3f &P,
                     const vec4f a,
                     const vec4f b,
                     const vec4f c,
                    ...
                    )
  {
   ...
   inline  __both__
  bool intersectWedge(vec3f &value,
                      const vec3f &P,
                      const vec4f _v0,
                      const vec4f _v1,
                      const vec4f _v2,
  ...
  {
    vec4f a = vertices[indices[0]];
    vec4f b = vertices[indices[1]];
    vec4f c = vertices[indices[2]];
    vec4f d = vertices[indices[3]];
                     ...
                     const ushort *indices)
  {
    ...
    vec4f b = vertices[indices[1]];
    vec4f c = vertices[indices[2]];
    vec4f d0 = vertices[indices[3]];
    vec4f d1 = vertices[indices[4]];
  ...
  bool intersectPyr(float &value,
                    ...
                    const ushort *indices)
  {
    const vec4f _v0 = vertices[indices[0]];
    const vec4f _v1 = vertices[indices[1]];
    const vec4f _v2 = vertices[indices[2]];
    const vec4f _v3 = vertices[indices[3]];
    const vec4f _v4 = vertices[indices[4]];
                      ...
                      const ushort *indices)
  {
    ...
    const vec4f _v3 = vertices[indices[3]];
    const vec4f _v4 = vertices[indices[4]];
    const vec4f _v5 = vertices[indices[5]];
  ...
  bool intersectHex(float &value,
                    ...
                    const ushort *indices)
  {
    vec4f v0 = vertices[indices[0]];
    vec4f v1 = vertices[indices[1]];
    vec4f v2 = vertices[indices[2]];
    vec4f v3 = vertices[indices[3]];
    vec4f v4 = vertices[indices[4]];
    vec4f v5 = vertices[indices[5]];
    vec4f v6 = vertices[indices[6]];
    vec4f v7 = vertices[indices[7]];
